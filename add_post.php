...
if (isset($_POST['publish']) || isset($_POST['draft'])) {
    ...
    if ($select_image->rowCount() > 0 && $image != '') {
        ...
        $insert_post = $conn->prepare("INSERT INTO `posts`(admin_id, admin_name, title, content, date, image, status) VALUES(?,?,?,?,?,?,?)");
        $insert_post->execute([$admin_id, $admin_name, $title, $content, $date, $image, $status]);
