...
function savePost($status)
{
    ...
    if (move_uploaded_file($image_tmp_name, $image_folder)) {
        ...
        $insert_post = $conn->prepare("INSERT INTO `posts`(admin_id, admin_name, title, content, image, status, mod_by) VALUES(?,?,?,?,?,?,?)");
        // Provide a value for `mod_by` field (you can set it to $admin_id or any other appropriate value)
        $insert_post->execute([$admin_id, $admin_name, $title, $content, $image, $status, $admin_id]);
...
<section class="post-editor">
   ...
   <br><br><br>
   <h1 class="heading">Add New Blog Post</h1>
   <br><br><br>
